...
function lang($un, $lang, $val = 0)
{
    ...
    if ($lang == 'ar') {
        if (is_numeric($un) && $un > 0 && $un == round($un, 0)) {
            $product_translation = DB::table('product_translation')->get()->where('Unique', $un)->where('language', $lang)->first();
            if ($val == 0) {
                echo $product_translation->Producer;
            } else {
                return  $product_translation->explained;
            }
        } else {
    ...
    //   $product_translation = DB::table('product_translation ')->get()->where('Name', 1)->where('language', 1)->first();
...
{
   // @dd("Catch errors for script and full tracking ( 6)".$code);
    $jsonString = [];
