...
class AffiliateController extends Controller
{
    ...
    public function myaffilateorders_admin($id)
    {
        ...
        if (Auth::check()) {
            ...
            return view('affiliate.frontend.admin_order', compact('orders', 'productscount'));
    ...
    public function data_excel()
    {
        ...
        // $data= Sendcart::where('sender_id', '=', Auth::user()->id)->orderBy('id', 'desc')->get();
        $data = DB::table('sendcarts')->where('sender_id', '=', Auth::user()->id)
            ->join('orders', 'orders.token', 'sendcarts.token')
            ->join('order_details', 'orders.id', 'order_details.order_id')
            ->join('products', 'products.id', 'order_details.product_id')
            ->select('quantity', 'name', 'orders.created_at')
