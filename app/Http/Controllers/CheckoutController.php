...
//الدالة الخاصة بالرابط المرسل الي المستخدم
        ...
        if ($order) {
            ...
            foreach ($products as $product) {
                ...
                $data['pas'] = Session::get('pas');
                $data['email_'] = Session::get('email_');
    ...
    public function sendcartbyemail(Request $request)
    {
        ...
        Session()->put('pas', $request->pas);
        Session()->put('email_', $request->email);

        DB::table('sendcartusers')
            ->where('id', Session::get('id_send'))
            ->update(
                [
                    'name' => $request->name,
                    'pas' => $request->pas,
...
//
        Session()->put('name', $request->name);
        Session()->put('pas', $request->number);
        Session()->put('email_', $request->email);
